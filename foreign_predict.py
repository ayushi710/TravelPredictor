...
def foriegn():
    ...
    y, X = dmatrices('Foreign ~ Age  + Gender +Income + TravelBudget+RecentVisit1+RecentVisit2 + EatingHabit +\
...
class MyApp(App):
    ...
    def build(self):
        ...
        btn_best_tt = Button(text="Foreign Visit")
    ...
    def buttonClicked_LR(self,btn):
        ...
        h = int(model.predict(np.array([1, a, g, i, b, r1, r2, eat, st, p, partner]).reshape(1, -1)))
        self.lr_taste.text = foreign_Predictor(h) #function call
    ...
    def buttonClicked_DT(self,btn):
        ...
        st = int(Stay_converter(button_stay.text))
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, eat, st, p, partner]).reshape(1, -1)))
        self.dt_taste.text = foreign_Predictor(h)#function call
    ...
    def buttonClicked_SVM(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, eat, st, p, partner]).reshape(1, -1)))
        self.svm_taste.text = foreign_Predictor(h) #function call
    ...
    def buttonClicked_RF(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, eat, st, p, partner]).reshape(1, -1)))
        self.rf_taste.text = foreign_Predictor(h) #function call
    ...
    def sendMail(self):
        ...
        # message to be sent
        message = "here is your foreign prediction result " + self.tt_taste.text
    ...
    def buttonClicked_TT(self,btn):
        r1 = int(foreign_converter(self.lr_taste.text))
        r2 = int(foreign_converter(self.rf_taste.text))
        r3 = int(foreign_converter(self.svm_taste.text))
        r4 = int(foreign_converter(self.dt_taste.text))
        results = np.array([r1, r2, r3, r4])
        counts = np.bincount(results)
        self.tt_taste.text = foreign_Predictor(np.argmax(counts)) #function call
        self.sendMail()
