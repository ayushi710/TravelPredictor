...
class MyApp(App):
    ...
    def build(self):
        ...
        """
        self.label10 = Label(text="Travel Place")
        layout.add_widget(self.label10)
        self.place_taste = TextInput(text='', multiline=False)
        layout.add_widget(self.place_taste)
        """
    ...
    def buttonClicked_LR(self,btn):
        ...
        h = int(model.predict(np.array([1, a, g, i, b, r1, r2, f, eat,  p, partner]).reshape(1, -1)))
        self.lr_taste.text = Stay_Predictor(h) #function call
    ...
    def buttonClicked_DT(self,btn):
        ...
        #t = int(TravelTaste_converter(self.place_taste.text))
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat,  p, partner]).reshape(1, -1)))
        self.dt_taste.text = Stay_Predictor(h) #function call
    ...
    def buttonClicked_SVM(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat,  p, partner]).reshape(1, -1)))
        self.svm_taste.text = Stay_Predictor(h) #function call
    ...
    def buttonClicked_RF(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat,  p, partner]).reshape(1, -1)))
        self.rf_taste.text = Stay_Predictor(h) #function call
    ...
    def sendMail(self):
        ...
        # message to be sent
        message = "here is your stay prediction result " + self.tt_taste.text
    ...
    def buttonClicked_TT(self,btn):
        r1 = int(Stay_converter(self.lr_taste.text))
        r2 = int(Stay_converter(self.rf_taste.text))
        r3 = int(Stay_converter(self.svm_taste.text))
        r4 = int(Stay_converter(self.dt_taste.text))
        results = np.array([r1, r2, r3, r4])
        counts = np.bincount(results)
        self.tt_taste.text = Stay_Predictor(np.argmax(counts)) #function call
        self.sendMail()
