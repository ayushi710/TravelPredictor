...
def partner_x():
    ...
    y, X = dmatrices('PreferredPartner  ~ Age + Gender +Income+ TravelBudget+RecentVisit1+RecentVisit2+ Foreign+ EatingHabit +\
                       Stay+ Purpose ', df, return_type='dataframe')
    y = np.ravel(y)
    return y, X
...
class MyApp(App):
    ...
    def buttonClicked_LR(self,btn):
        ...
        #t = int(TravelTaste_converter(self.place_taste.text))
        st = int(Stay_converter(button_stay.text))
        h = int(model.predict(np.array([1, a, g, i, b, r1, r2, f, eat, st, p]).reshape(1, -1)))
        self.lr_taste.text = PrefferedPartner_Predictor(h) #function call
    ...
    def buttonClicked_DT(self,btn):
       ...
       # t = int(TravelTaste_converter(self.place_taste.text))
        st = int(Stay_converter(button_stay.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat, st, p]).reshape(1, -1)))
        self.dt_taste.text = PrefferedPartner_Predictor(h)  # function call
    ...
    def buttonClicked_SVM(self,btn):
        ...
        st = int(Stay_converter(button_stay.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat, st, p]).reshape(1, -1)))
        self.svm_taste.text = PrefferedPartner_Predictor(h)
    ...
    def buttonClicked_RF(self,btn):
        ...
        st = int(Stay_converter(button_stay.text))
        h = int(clf.predict(np.array([1, a, g, i, b, r1, r2, f, eat, st, p]).reshape(1, -1)))
        self.rf_taste.text = PrefferedPartner_Predictor(h)
    ...
    def sendMail(self):
        ...
        # message to be sent
        message = "here is your partner prediction result " + self.tt_taste.text
    ...
    def buttonClicked_TT(self,btn):
        r1 = int(PrefferedPartner_converter(self.lr_taste.text))
        r2 = int(PrefferedPartner_converter(self.rf_taste.text))
        r3 = int(PrefferedPartner_converter(self.svm_taste.text))
        r4 = int(PrefferedPartner_converter(self.dt_taste.text))
        results = np.array([r1, r2, r3, r4])
        counts = np.bincount(results)
        self.tt_taste.text = PrefferedPartner_Predictor(np.argmax(counts)) #function call
        self.sendMail()
