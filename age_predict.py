...
def age():
    ...
    y, X = dmatrices('Age ~ Gender +Income+ TravelBudget+RecentVisit1+RecentVisit2+ Foreign+ EatingHabit +\
...
class MyApp(App):
    ...
    def buttonClicked_LR(self,btn):
        ...
        h = int(model.predict(np.array([1, g, i, b, r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
    ...
    def buttonClicked_DT(self,btn):
        ...
        st = int(Stay_converter(button_stay.text))
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, g, i, b, r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
    ...
    def sendMail(self):
        ...
        # message to be sent
        message = "here is your age prediction result " + self.tt_taste.text
