...
def budget():
    ...
    y, X = dmatrices('TravelBudget ~ Age + Gender +Income+RecentVisit1+RecentVisit2+ Foreign+ EatingHabit +\
...
class MyApp(App):
    ...
    def build(self):
        ...
        btn_best_tt = Button(text="Budget")
    ...
    def buttonClicked_LR(self,btn):
        ...
        h = int(model.predict(np.array([1, a, g, i,r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
        self.lr_taste.text = str(h)  #function call
    ...
    def buttonClicked_DT(self,btn):
        ...
        st = int(Stay_converter(button_stay.text))
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
        self.dt_taste.text = str(h) #function call
    ...
    def buttonClicked_SVM(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
        self.svm_taste.text = str(h) #function call
    ...
    def buttonClicked_RF(self,btn):
        ...
        partner = int(PrefferedPartner_converter(button_partner.text))
        h = int(clf.predict(np.array([1, a, g, i, r1, r2, f, eat, st, p, partner]).reshape(1, -1)))
        self.rf_taste.text = str(h) #function call
    ...
    def sendMail(self):
        ...
        # message to be sent
        message = "here is your budget prediction result " + self.tt_taste.text
    ...
    def buttonClicked_TT(self,btn):
        r1 = int(self.lr_taste.text)
        r2 = int(self.rf_taste.text)
        r3 = int(self.svm_taste.text)
        r4 = int(self.dt_taste.text)
        results = np.array([r1, r2, r3, r4])
        counts = np.bincount(results)
        self.tt_taste.text = str(np.argmax(counts))
        self.sendMail()
